...
import { GEM_TYPES, GEM_TINTS } from '../systems/GemFactory.js';
...
export class GameScene extends Phaser.Scene {
  ...
  _buildLevelConfig(levelIndex) {
    const targetGemType = levelIndex % GEM_TYPES.length;
    return {
      targetGemType,
  ...
  create() {
    ...
    this._createSceneBackdrop();

    this.board = new Match3Board(ROWS, COLS, GEM_TYPES.length);
  ...
  // Ekranın tamamını kaplayan dekoratif sahne arka planı (varsa panel_wood, yoksa
  // dünyanın arka plan illüstrasyonu). Bu görsel, tahtaya sıkıştırılmaz — bütün ekranı
  // kaplayacak şekilde "cover" mantığıyla ölçeklenir, tıpkı orijinal tasarımdaki gibi.
  _createSceneBackdrop() {
    const { width, height } = this.scale;
    const key = this.textures.exists('panel_wood') ? 'panel_wood' : `bg_${this.worldId}`;
    if (!this.textures.exists(key)) return;

    const img = this.add.image(width / 2, height / 2, key).setDepth(-10);
  ...
  _createHUD() {
    ...
    const goalY = 50;
    this.add.image(30, goalY, `gem_${this.levelConfig.targetGemType}`).setDisplaySize(32, 32).setOrigin(0.5);
  ...
  _createGridBackground() {
    const padding = 16;
    // Ahşap görseli artık _createSceneBackdrop() içinde tam ekran dekor olarak kullanılıyor.
    // Burada sadece tahtanın okunabilirliği için hafif koyu bir kontrast paneli çiziyoruz.
    const g = this.add.graphics();
    g.fillStyle(0x000000, 0.32);
    g.fillRoundedRect(this.gridX - padding / 2, this.gridY - padding / 2, this.gridW + padding, this.gridH + padding, 14);
  }
  ...
  _popMatches(matches, multiplier) {
    return new Promise(resolve => {
      matches.forEach(({ r, c, type }, i) => {
        ...
        const { x, y } = this._cellToXY(r, c);
        this.sparkleEmitter.setParticleTint(GEM_TINTS[type]);
  ...
  _animateSpawns(spawns) {
    spawns.forEach(({ row, col, type }) => {
      ...
      const img = this.add.image(x, y - this.cellSize * (ROWS), `gem_${type}`)
        .setDisplaySize(this.cellSize * 0.82, this.cellSize * 0.82)
        .setInteractive({ useHandCursor: true });
