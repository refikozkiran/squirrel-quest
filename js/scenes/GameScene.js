...
export class GameScene extends Phaser.Scene {
  ...
  _createHUD() {
    ...
    const goalY = 50;
    this.add.image(30, goalY, `gem_${this.levelConfig.targetGemType}`).setScale(0.35).setOrigin(0.5);
  ...
  _createGridBackground() {
    const padding = 16;
    const panelKey = 'panel_wood';

    // Gerçek ahşap panel dokusu (assets/ui/panel-wood.png) yüklenmişse onu kullan.
    if (this.textures.exists(panelKey)) {
      const img = this.add.image(this.gridX + this.gridW / 2, this.gridY + this.gridH / 2, panelKey);
      img.setDisplaySize(this.gridW + padding * 2, this.gridH + padding * 2);
      return;
    }

    // Yoksa: sade prosedürel panel.
